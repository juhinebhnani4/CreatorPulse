...
<body>
    ...
    def generate_newsletter(
        ...
        items = self._select_diverse_content(
            content_items,
            max_items=max_items,
            max_per_source=max(10, max_items)  # Allow more items if they're high quality
        )
    ...
    def _generate_content_with_ai(
        ...
        try:
            ...
            # Check if it's a quota/rate limit error (429) and we can fallback to OpenRouter
            if ("429" in error_str or "quota" in error_str.lower() or "rate_limit" in error_str.lower()):
                # Only attempt OpenRouter fallback if:
                # 1. We're currently using OpenAI (not already using OpenRouter)
                # 2. OpenRouter API key is configured
                if not self.config.use_openrouter and self.config.openrouter_api_key:
                    self.logger.warning("OpenAI quota exceeded. Attempting fallback to OpenRouter...")
                    try:
                        ...
                        self.logger.info(f"Retrying with OpenRouter (model: {openrouter_model})")
                        ...
                        content = response.choices[0].message.content
                        self.logger.info("Successfully generated newsletter using OpenRouter fallback")
                        return self._parse_ai_response(content)
                    except Exception as fallback_error:
                        self.logger.error(f"OpenRouter fallback also failed: {fallback_error}")
                        # Fall through to fallback content
                else:
                    if self.config.use_openrouter:
                        self.logger.error("Already using OpenRouter - no fallback available")
                    else:
                        self.logger.error("OpenRouter API key not configured - cannot fallback")

            # Return fallback content if AI generation completely fails
            self.logger.warning("Using fallback content generation (no AI)")
            fallback_content_html = self._generate_fallback_content(items)
            fallback_result = {
                ...
                'content': fallback_content_html,
                'footer': footer or "Thank you for reading! Generated by CreatorPulse."
            }
            self.logger.info(f"[FALLBACK_RESULT] Returning fallback with content length: {len(fallback_content_html)} chars")
            return fallback_result
    
    def _generate_fallback_content(self, items: List[ContentItem]) -> str:
        """Generate fallback content when AI generation fails."""
        self.logger.info(f"[FALLBACK] Generating fallback content for {len(items)} items")
        content_html = ""
        ...
        self.logger.info(f"[FALLBACK] Generated {len(content_html)} chars of HTML content")
        self.logger.debug(f"[FALLBACK] Content preview: {content_html[:500]}...")
        return content_html
...
"""
        ...
        prompt = f"""
You are an expert newsletter writer creating a high-quality, human-curated AI briefing. Your goal is to transform raw content into an insightful, narrative-driven newsletter that feels professionally edited, not robotic.

{items_text}

CRITICAL INSTRUCTIONS - YOU MUST FOLLOW THESE:

1. **IDENTIFY THEMES & CREATE SECTIONS**: Analyze all {total_items} items and identify 3-5 major themes or topics. Group related stories together. DO NOT just list items sequentially.

2. **SPOT TRENDS ACROSS SOURCES**: Look for patterns like:
   - Multiple sources discussing the same topic (e.g., "Three companies announced major AI infrastructure investments this week...")
   - Emerging debates or controversies
   - Shifts in industry direction
   - Common concerns or excitement

3. **ADD CONTEXT & ANALYSIS**: For each section:
   - Explain WHY this matters (not just WHAT happened)
   - Connect the dots between related stories
   - Provide brief industry implications
   - Use phrases like "This signals...", "The trend suggests...", "What this means for..."

4. **MERGE SIMILAR TOPICS**: If multiple items cover the same event/topic from different sources, synthesize them into ONE cohesive discussion (not separate items).

5. **NARRATIVE FLOW**:
   - Start with a contextual intro paragraph that sets the scene
   - Use transitional phrases between sections
   - Create a story arc (setup → development → implications)
   - End with forward-looking insights

6. **STRUCTURE EACH SECTION**:
   ```
   ## [Number]. [Thematic Section Title]

   [2-3 sentences of context/analysis explaining the theme]

   [Synthesized discussion of relevant stories with inline citations]
...
7. **TONE**: Write like a knowledgeable insider briefing a colleague. Confident, journalistic, insightful - not academic or promotional.

Please provide the newsletter in the following JSON format:
{{
    "title": "{title or 'Your Daily Tech Digest'}",
    "intro": "A narrative intro paragraph (3-4 sentences) that sets context, highlights the biggest story, and previews key themes. Use phrases like 'In a move that...', 'This week saw...', 'As the industry grapples with...'",
    "content": "HTML content organized into 3-5 numbered thematic sections (see structure above). Each section should feel like a mini-article with analysis, not just a list of links.",
    "footer": "A brief forward-looking closing (1-2 sentences) that hints at what to watch next"
}}

EXAMPLE SECTION STRUCTURE:
```html
<h2>1. The AI Compute Arms Race Heats Up</h2>
<p>In a week that underscored the massive capital requirements of the AI era, three major players announced billion-dollar infrastructure deals. This spending spree reflects a growing industry consensus: whoever controls compute capacity in 2025 will define the competitive landscape for the next decade.</p>

<p>Meta led the charge with a <a href="[url]">$37 billion financing commitment</a> through Blue Owl Capital, earmarked for a sprawling Louisiana data center project. The deal marks one of the largest private infrastructure investments in tech history and signals Meta's determination to reduce dependence on cloud providers. Meanwhile, OpenAI is <a href="[url]">reportedly in talks with Google</a> to secure additional compute capacity, despite its close partnership with Microsoft. The move suggests even well-funded AI leaders are feeling the squeeze.</p>

<p><strong>Key developments:</strong></p>
<ul>
    <li>Meta's <a href="[url]">$37B Blue Owl financing deal</a> for Louisiana data center</li>
    <li>OpenAI <a href="[url]">exploring Google cloud partnership</a> alongside Microsoft</li>
    <li>Anthropic weighing <a href="[url]">in-house chip development</a> to reduce Nvidia dependence</li>
</ul>

<p style="background: #f0f9ff; padding: 15px; border-left: 4px solid #3b82f6; margin-top: 15px;"><strong>💡 What this means:</strong> The scramble for compute capacity is rapidly becoming AI's defining constraint. Companies that secure infrastructure now are positioning themselves for long-term competitive advantage, while others risk being priced out of cutting-edge model development.</p>
```
...
- Include HTML formatting for emphasis, sections, and callout boxes
- Link to sources inline, not as separate items
"""
    ...
    def _parse_ai_response(self, content: str) -> Dict[str, str]:
        ...
        self.logger.info(f"[_parse_ai_response] Parsing AI response (length: {len(content)})")
        self.logger.debug(f"[_parse_ai_response] First 500 chars: {content[:500]}")

        # [DEBUG] DIAGNOSTIC: Print full AI response to console
        print("\n" + "="*80)
        print("[DEBUG] FULL AI RESPONSE FROM MODEL:")
        print("="*80)
        print(content)
        print("="*80 + "\n")
        ...
        try:
            ...
            if start_idx != -1 and end_idx != 0:
                ...
                self.logger.info(f"[_parse_ai_response] [OK] Successfully parsed JSON with keys: {list(parsed.keys())}")

                # DIAGNOSTIC: Check content field
                if 'content' in parsed:
                    content_preview = parsed['content'][:500] if len(parsed['content']) > 500 else parsed['content']
                    self.logger.info(f"[_parse_ai_response] Content field length: {len(parsed['content'])}")
                    print(f"\n[DEBUG] AI Content Field Preview (first 500 chars):\n{content_preview}\n")
                else:
                    self.logger.warning("[_parse_ai_response] [WARNING] JSON missing 'content' field!")
                    print(f"\n[WARNING] JSON MISSING 'content' FIELD! Keys found: {list(parsed.keys())}\n")

                return parsed
            else:
                # Fallback: treat entire response as content
                self.logger.warning("[_parse_ai_response] [WARNING] No JSON found, using entire response as content")
                print("\n[WARNING] NO JSON BRACES FOUND - Using fallback (entire response as content)\n")
                return {
            ...
            # Fallback parsing
            self.logger.warning(f"[_parse_ai_response] [WARNING] JSON decode error: {e}, using fallback")
    ...
    def _format_html(self, generated_content: Dict[str, str], items: List[ContentItem], trends: Optional[List[Dict[str, Any]]] = None) -> str:
        ...
        # Use AI-generated content if available, otherwise fallback to manual formatting
        ai_content = generated_content.get('content', '')
        final_content = trending_html + (ai_content if ai_content else content_html)

        self.logger.info(f"[FORMAT_HTML] AI content length: {len(ai_content)} chars")
        self.logger.info(f"[FORMAT_HTML] Manual content_html length: {len(content_html)} chars")
        self.logger.info(f"[FORMAT_HTML] Final content length: {len(final_content)} chars")
        self.logger.debug(f"[FORMAT_HTML] Final content preview: {final_content[:300]}...")

        html = html.replace('{{content}}', final_content)
        html = html.replace('{{footer}}', generated_content.get('footer', 'Thanks for reading!'))

        self.logger.info(f"[FORMAT_HTML] Final HTML length: {len(html)} chars")
        return html
