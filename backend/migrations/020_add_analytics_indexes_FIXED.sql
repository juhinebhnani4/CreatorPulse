-- =============================================================================
-- Migration 020: Add Analytics Performance Indexes (FIXED)
-- =============================================================================
-- Description:
...
--
--   FIXED: Corrected pg_stat_user_indexes query column names for Supabase
--
...
-- Verification query - Show index sizes and usage potential (FIXED)
-- Note: pg_stat_user_indexes uses 'relname' not 'tablename' in some PostgreSQL versions
SELECT
    'Migration 020 Complete' as status,
    schemaname,
    relname as tablename,  -- FIXED: Use relname instead of tablename
    indexrelname as indexname,  -- FIXED: Use indexrelname instead of indexname
    pg_size_pretty(pg_relation_size(indexrelid)) as index_size,  -- FIXED: Use indexrelid
    idx_scan as times_used,
    idx_tup_read as tuples_read,
    idx_tup_fetch as tuples_fetched
FROM pg_stat_user_indexes
WHERE relname = 'content_items'  -- FIXED: Use relname
    AND indexrelname LIKE 'idx_content_%'  -- FIXED: Use indexrelname
ORDER BY indexrelname;

-- Show table size comparison (FIXED)
SELECT
    ...
    (
        SELECT COUNT(*)
        FROM pg_indexes
        WHERE tablename = 'content_items'  -- This is correct for pg_indexes view
